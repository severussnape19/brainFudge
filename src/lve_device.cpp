...
LveDevice::LveDevice(LveWindow &window) : window{window} {
  createInstance(); // initializes the vulkan library and creates a connectionw with our application and vulkan 
  setupDebugMessenger(); // validation layers. vulkan does very little error checking.. so even the smallest issue can crash the program.
  // Error checking is used for production and later removed for the release build 
  createSurface();  // connection between the window and vulkan's ability to display results
  pickPhysicalDevice(); // Graphics device
  createLogicalDevice(); // Describes what features of our physical device we want to use
  createCommandPool(); // for command buffer allocation
}
